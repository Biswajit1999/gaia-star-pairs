...
def sync(results_dir: Path, figures_dir: Path, manifest_path: Path, source_catalog_path: Path, web_public_dir: Path) -> None:
    ...
    if source_catalog_path.is_file():
        shutil.copy2(source_catalog_path, web_public_dir / "source_catalog.csv")
        copied.append(str(source_catalog_path))
...
def main() -> None:
    ...
    sync(args.results_dir, args.figures_dir, args.manifest, args.source_catalog, args.web_public_dir)
